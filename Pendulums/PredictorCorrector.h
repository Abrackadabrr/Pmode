...
namespace ODE_Solvers {
    ...
    std::vector<value_t> predict_correct_next_step(const vector_function& func, std::vector<value_t>& state, value_t time, value_t timestep) {
        auto current_state = state;
        state = ODE_Solvers::eiler_next_step(func, state, time, timestep);
        return std::move(current_state + func(state, time)*timestep);
    }

    /*
     * Based on predict_correct_next_method
     * Make n_iters iterations of method
     * Make sure that arguments of vector_function is exactly std::vector<value_t> and time
     * initial_state is initial state of a system
     */
    template<typename value_t, typename vector_function>
    std::vector<std::vector<value_t>> predict_correct_method(vector_function func, const std::vector<value_t>& initial_state,
        ...
        for (int i = 0; i < n_iters; ++i) {
            state = ODE_Solvers::predict_correct_next_step(func, state, time, timestep);
            time += timestep;
            result.push_back(state);
        }
